...
\end{itemize}
						...
						\vskip.3in
						Utility comparisons (always for high type)
							\begin{itemize}
...
\subsubsection{To do and Notes}		
	\color{ForestGreen} 
			\begin{itemize}
				\item Obviously we can come up with other parameterizations in which this may look a little more reasonable
					\begin{itemize}
						\item But I think the first step is to calculate the high-type utility under the different scenarios and the different gifts and see what happens (since you get the indirect benefit of the gift--or do you since $e$ is too small?)
					\end{itemize}
				\item When g1 turns negative, is that exactly when $e=1$ can't be sustained? If we can get positive $g1$, not sure we'd need the mediator? It's not about $g1$, but the $\de_h^1$ condition.
					\begin{itemize}
						\item When g1 is negative, the constraint on $\de_h^1$ (see proof of Thm 3) becomes an upper bound. If it's negative, it means that no $\de_h$ exists to make separating possible.
					\end{itemize}
			\end{itemize}
