...
\end{itemize}
						...
						Welfare comparisons (always for high type)
							\begin{itemize}
								\item When $e=1$: $\frac{1}{1-d_h}\left\{pT(1+g^*) + (1-p)(W-D(1+g^*))\right\} - \frac{p(1-d_l)(D+T)}{(1-d_l)(1-pT)+(1-p)D}$
									\begin{itemize}
										\item \color{red} Parameterization in the paper: welfare is 7.18; gift is 0.63
										\item \color{orange} Parameterization 2: $g^1$ is negative, creates negative upper bound for $d_h$. Separating with concessions equilibrium can't be sustained.
									\end{itemize}
								\item When $e=0$: $\frac{1}{1-d_h}\left\{pT + (1-p)(W-D)\right\} - p(T+D)$
									\begin{itemize}
									...
									\end{itemize}
								\item When $0<e<1$ under a mediator: $\frac{1}{1-d_h}\left\{pT(1+eg_M) + (1-p)(W-D)\right\} - g_M$
									\begin{itemize}
...
$$\frac{1}{1-d_h}\left\{pT(1+eg_M) + (1-p)(W-D)\right\} - \frac{\left(1 - \de_l \right) \left( T + D \right)}{p - e \left( p + T - \de_l T + W \right)}$$

When $e=0$, welfare is $$\frac{1}{1-d_h}\left\{pT + (1-p)(W-D)\right\} - p(T+D)$$

We compare them to find when the welfare under the mediator is larger:
	\begin{multline*}
		\frac{1}{1-d_h}\left\{pT(1+eg_M) + (1-p)(W-D)\right\} - \frac{\left(1 - \de_l \right) \left( T + D \right)}{p - e \left( p + T - \de_l T + W \right)} \geq \\
		\frac{1}{1-d_h}\left\{pT + (1-p)(W-D)\right\} - p(T+D)$$
	\end{multline*}							
Eliminating common terms:
	\begin{equation*}
		\frac{1}{1-d_h}\left\{pTeg_M \right\} - \frac{\left(1 - \de_l \right) \left( T + D \right)}{p - e \left( p + T - \de_l T + W \right)} \geq - p(T+D)
	\end{equation*}	
Rearranging:
	\begin{equation*}
		\frac{pTeg_M}{1-d_h} \geq \frac{\left(1 - \de_l \right) \left( T + D \right)}{p - e \left( p + T - \de_l T + W \right)} - p(T+D)
	\end{equation*}	
Here we can clearly see the increase in welfare from the impact of the effective gift on the left has to be greater than the increase in the size of the gift.
	\begin{itemize}
		\item \color{blue} I'm not sure anything reasonable can come from this
	\end{itemize}
...
\subsubsection{To do and Notes}		
	\color{ForestGreen} 
			\begin{itemize}
					\item NEXT STEP: check condition for sustainable $e=1$ but prefer mediator with $e < 1$ (I've found parameterizations where $e=1$ is preferred so far, but I derived an expression in my notes for when mediator is better $$g^M(pTe - 1) \geq g^1(pT + pD - D - 1)$$
						\begin{itemize}
							\item I'll probably need to type these notes up
						\end{itemize}
