						...
						\begin{itemize}
							\item $T+D$, $1 - \de $ non-negative by assumption $\Rightarrow$ RHS of Expression~\ref{eq:g1} non-negative
								\begin{itemize}
									...
									\item SO WE MUST ASSUME {\color{red} $p - e \left( p + T - \de T + W \right)$ IS POSITIVE}
								\end{itemize}
...
\begin{itemize}
	\item This is all in the paper now.
\end{itemize}

How to choose the optimal $e$ when both the low type and high type IC bind is still done informally in R.

								
\subsubsection{Welfare under Mediator vs. CSE with inefficient gifts}
\label{sec:WM0}
I tried to derive a necessary condition relating the IC/IR constraints to this welfare comparison but didn't find anything easily. I think I explored this in Mathematica, but I'd need to look more carefully through the Mathematica files.
...
\vskip.2in
Parameterization 5: $We > W1 > W0$. Asymptote at $e = 03759...$ (Was incorrect before I understood the asymptote: $W1 > W0 > We$ (mediator). Mediator optimally chooses $e=0.03$(
\begin{itemize}
	\item $T = 6, \ W = 8, \ D = 5, \ p = .5, \ \de_l = .2, \ \de_h = .9$
\end{itemize}

\vskip.2in
Parameterization 6: $We > W1 > W0$, $54 > 7.33 > 7$. Mediator optimally chooses $e=0.24$ (0.25 is the asymptote). Optimal gift is 50, effective gift is 12, vs. $g1 = .667$ and $g0 = 1$
\begin{itemize}
	\item $T = 1, \ W = 1, \ D = 1, \ p = .5, \ \de_l = .5, \ \de_h = 15/16=0.9375$
\end{itemize}
	
\vskip.2in
Parameterization 7: $We > W0 > W1$, $42.33 > 4.96 > 2.83$. Mediator optimally chooses $e=0.28$ (0.28571 is the asymptote). Optimal gift is 75, effective gift is 21, vs. $g1 = .428$ and $g0 = .75$
\begin{itemize}
	\item $T = 0.5, \ W = 1, \ D = 1, \ p = .5, \ \de_l = .5, \ \de_h = 61/64=0.953125$
\end{itemize}
...
\subsubsection{To do and Notes}		
	\color{ForestGreen} 
			\begin{itemize}
					\item Need to say clearly that $e=1$ doesn't work in a lot of situations
					\item Can I do new mediator theorem in the same form as the one that's currently in the paper?
					\item Mediator with $e=0$ can be better than CSE with $e=0$. Need to say why this is
						...
						\end{itemize}
					\item If you can sustain $e=1$ with no mediator, mediator can still be better
						\begin{itemize}
							\item Again, if mediator is good, it's REALLY good.
							\item The benefit of $eg_M$ is muted versus $g^1$, but there is no harm from $g_M$ while there is harm from $g^1$. Cost of gift will always be larger under mediator because it goes to infinity at the optimal $e$
						\end{itemize}
					\item Sufficient condition for where welfare goes to positive $\infty$ to the left of the $e$ asymptote
						\begin{itemize}
							\item Characterizing exactly the region where Welfare goes to positive $\infty$ to the left of the $e$ asymptote seems hard. But I have developed an intuitive sufficient condition. See end of Section~\ref{sec:WM1}
						\end{itemize}
			\end{itemize}
