...
\]
	\begin{itemize}
		...
		\item From here we can solve either for the constraint in terms of $g$ or in terms of $e$
							...
							\begin{itemize}
								...
								\item Parameterization that is in the paper now:
									\begin{itemize}
										...
										\item Threshold for high types to separate when concessions do not have future material value is $\de = \frac{8}{10+5} = .5\overline{3}$. Let $\de_h = .6$ and $\de_l = .3$. Assume $p = .2$. By Theorem 2, separation through concessions is possible.
			...
			\end{itemize}
		\item \color{ForestGreen} No notes to digitize right now
			\begin{itemize}
				\item Calculate maximum $e$ from Expression~\ref{eq:g6} using parameterization from paper
					\begin{itemize}
						\item Do it under simple $g$ and the more complicated $g$ from Expression~\ref{eq:g4}
						\item Compare to calculations in example from text
						\item Try another parameterization with $p$ larger so it works
					\end{itemize}
