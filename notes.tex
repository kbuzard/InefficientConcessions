...
\un{November 29-30, 2020}: In making revisions in April 2020 to respond to the EJPE referee reports, I discovered the IC constraint does not hold for the low type in Section 5: Mediation. Here I will see what adjustments might make the mediator work.\\
...
\subsection{Why does low type IC not hold when $e=1$?}
$$X_{FF}^l \geq pX_{FT}^l+(1-p)X_{FF}^l-pg_h+peg_h $$
\begin{itemize}
	\item With our restrictions (mostly the linear assumption, IIRC), High type gives gift with efficiency $e=1$, so gift giving and receiving cancels out. This leaves
		...
		We've already assumed $e=1$ here as given in Lemma 2 for a concessions separating equilibrium 
			\begin{itemize}
				\item \textbf{Mediator could instruct them to give $e<1$ gifts without having to disturb the rest of the theory (I think). This I'll come back to.}
				...
				\item We also know that there are no gifts exchanged for $X^l_{FF}$, so $g_1 = g_2 = 0$ on the LHS. On the RHS, gifts ARE exchanged because player 2 is a high type and the low type of player 1 lied. So we can set $g_1 = g_2 = g_M$
					...
					\end{multline*}
				\item Now for the $\al$'s. The low type of player 1 sets $\al_1 = 0$ (this is the amount that goes to civil society). Player 2 is a high type here, so sets $\al_2 = 1$.
					\[
...
*** THIS IS THE NEXT STEP IN THE ANALYSIS *** Here we would be using inefficient gifts to get the low type to truthfully reveal.
