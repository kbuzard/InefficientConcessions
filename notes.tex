						...
						Utility comparisons (always for high type)
							\begin{itemize}
								\item When $e=1$: $\frac{1}{1-d_h}\left\{pT(1+g^*) + (1-p)(W-D(1+g^*))\right\} - \frac{p(1-d_l)(D+T)}{(1-d_l)(1-pT)+(1-p)D}$
									\begin{itemize}
										\item \color{red} For the parameterization in the paper, welfare is 7.18; gift is 0.63
									\end{itemize}
								\item When $e=0$: $\frac{1}{1-d_h}\left\{pT + (1-p)(W-D)\right\} - p(T+D)$
									\begin{itemize}
										\item \color{red} For the parameterization in the paper, welfare is 8; gift is 3
									\end{itemize}
								\item When $0<e<1$ under a mediator: $\frac{1}{1-d_h}\left\{pT(1+eg_M) + (1-p)(W-D)\right\} - g_M$
									\begin{itemize}
										\item Figure out optimal $g_M$
										\item Expression~\ref{eq:g2} creates a menu of $(e,g)$ pairs that satisfy the constraint:
											\[
...
\subsubsection{To do and Notes}		
	\color{ForestGreen} No notes to digitize right now
			\begin{itemize}
				\item Check parameter restrictions for existence of equilibria in Section 4.2
					\begin{itemize}
						\item Go back to 4 or 4.1; didn't find them in 4.2
					\end{itemize}
