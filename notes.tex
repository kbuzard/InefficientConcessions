	...
	\color{ForestGreen} 
			\begin{itemize}
					\item Can I derive an analytical expression for when the mediator enhances welfare? See Section~\ref{sec:WM0}
						\begin{itemize}
							\item If you can sustain $e=1$ with no mediator, would you ever want a mediator? YES
					...
					\item Minimum $\de$ for $e=0$ CSE / no material value case to be sustainable. Remember that $g^0 = p(T+D)$
						...
						\]
							...
							\end{itemize}
						The following shows that this is strictly smaller than $\de^h$. This means that the set of types who can make and sustain peace under the CSE with no material value / CSE with material value and $e=0$ is larger than the set of types who can make and sustain peace under the repeated game.
							\begin{itemize}
								\item $T+D>W$
								\item $-(T+D)<-W$
								\item $-p(T+D)<-pW$
