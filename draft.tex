...
%\begin{document}\title{Inefficient Concessions and Mediation} 
%\author{ Ben Horne}
...
%\section{Abstract}
%We explore a model of bilateral conflict, examining the role of concessions as signals, in addition to being ends in themselves. In doing so, we open up a rationale for mediation: to remove the inefficiencies of signaling in the pre-play round of negotiations. Two parties are engaged in conflict, manifested as the stage game Nash equilibrium of a repeated Prisoner's Dilemma. Each party distrusts the other's willingness to pursue peace, and can indicate its own willingness through costly signaling, i.e. concessions. In resolving this trust issue and attempting to end the conflict, efficient concessions are generally preferred, but optimal signaling can include inefficiency if concessions can later be used against the giving party.  Under such scenarios, a third party mediator can sometimes reduce inefficiency and improve welfare, and we give an explanation for the mechanism that makes mediation effective.
%Two parties are engaged in conflict, and each distrusts the other's willingness to pursue peace. Each can indicate its own interest in peace through costly signaling, or concessions. In certain environments, however, a lack of trust means that optimal concessions are not made, creating inefficiencies in the negotiation that can reduce the possibility of peace. Through a formal model, we explore ways in which a third party mediator can act as a guarantor that promised concessions will be delivered, thereby reducing inefficiencies and increasing the potential for peace. In this process, we open up a rationale for mediation: to remove the inefficiencies of signaling in a preliminary round of negotiations.
 %   Results show that in bilateral negotiation, parties may prefer to give inefficient concessions due to the other party’s potential power to hurt. Additionally, conditions are shown under which a party be willing to, or even prefer to, give the first concession rather than waiting or giving simultaneously.    ...gain insight into the usefulness of concessions in making peace between warring factions. 
%\newpage

\section{Introduction and motivation} %LAKE For instance, the problem you begin with in the cases of Israel/Syria and West Bank are well understood as bargaining over assets that confer future power. But this is not understood as a problem of private information, as you treat it here, but as one of credible commitment. Its not clear how your types map onto the commitment problem. HOW: THAT TYPES COULD CHANGE OVER TIME?
