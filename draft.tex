...
\vskip.1in
\begin{theorem}
	Assume the high types are not patient enough to separate without concessions. There are parameters under which the ability of the low-type country to use concessions to reduce the payoffs of its negotiating partner during a 'fight' stage (i.e. $\al_L < 1$) destroys the negotiating partners' ability to separate through concessions, i.e. to achieve peace.
...
\begin{theorem}
	It is optimal to give efficient gifts (where $e=1$) in a separating equilibrium.
	\label{theorem:3}
\end{theorem}
\emph{Proof:}
