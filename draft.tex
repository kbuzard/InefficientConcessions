...
\emph{Mediation}
\begin{enumerate}
		\item[(a)] The mediation mechanism $M$ is incentive compatible for the low type when the specified concession is greater than or equal to $\frac{\left(1 - \de_l \right)\left(T +D \right)}{p - e\left(p +(1-\de_l)T +W \right)}$ and $p > e\left(p+\left(1-\de_l\right)T + W\right)$.
		\item[(b)] The mediation mechanism $M$ is incentive compatible for the high type when
			\begin{itemize}
				\item[(i)] $\de_h(T+D)\geq W$ and $e(T-W) >1 -\de_h$;
				\item[(ii)] $\de_h(T+D)\geq W$, $e(T-W) <1 -\de_h$ and the specified concession is less than or equal to $\frac{\de_h(T+D)-W}{1-\de_h - e\left(T +W \right)}$; or
				\item [(iii)] $\de_h(T+D)\leq W$, $e(T-W) >1 -\de_h$ and the specified concession is greater than or equal to $\frac{\de_h(T+D)-W}{1-\de_h - e\left(T +W \right)}$.
			\end{itemize}
		\item[(c)] The mediation mechanism $M$ is always individually rational for the low type. If $Te > (1-\de_h)(1-e)$, the high type's individual rationality constraint implies an additional restriction that the specified concession is less than or equal to $\frac{ T + D - W}{Te - (1-\de_h)(1-e)}$.
\end{enumerate}
	\label{theorem:med}
\end{theorem}
\emph{Proof:} See Appendix.
...
The welfare target is again the sum of the high type utilities. Qualitatively, similar results would hold if weighted high- and low-type welfare by the relevant probabilities. 
...
(a) For the general form of the low type incentive compatibility constraint, we start from Expression~\ref{eq:med} and add back in the $e$'s that were removed when assuming $e=1$ in the proof of Theorem~\ref{theorem:med}. So we start from
\begin{equation*}
			W-D \geq 
...
It is always optimal for the high type to invest in civil society when they truthfully reveal and to invest in the military when they lie about their type. Thus the constraint becomes
\begin{multline*}
  \frac{p}{1-\de_h}\left(T + Teg \right) - pg +peg+ \frac{1-p}{1-\de_h}\left(W-D\right) \geq \\
	p\left(T + W + Weg +eg + \frac{\de_h}{1-\de_h}\left(W + Weg -D\right)\right) + \frac{1-p}{1-\de_h}\left(W-D\right)
\end{multline*}
This simplifies to 
\begin{equation*}
  \frac{1}{1-\de_h}\left(T + Teg \right) - g \geq T + W + Weg + \frac{\de_h}{1-\de_h}\left(W + Weg -D\right)
\end{equation*}
%\begin{equation*}
%  T + Teg - ( 1-\de_h)g \geq ( 1-\de_h)\left(T + W + Weg\right) + \de_h\left(W + Weg -D\right)
%\end{equation*}
\begin{equation*}
  T + Teg - ( 1-\de_h)g \geq ( 1-\de_h)T + W + Weg - \de_h D
\end{equation*}
%\begin{equation*}
%  Teg - Weg -( 1-\de_h)g \geq -\de_h T + W - \de_h D
%\end{equation*}
\begin{equation}
  g\left(Te - We -( 1-\de_h)\right) \geq W - \de_h (D+T)
	\label{eq:med4}
\end{equation}
We have four cases to consider depending on signs:
\begin{itemize}
	\item [(i)] When $\de_h(T+D)\geq W$ and $e(T-W) >1-\de_h$, then we have a lower bound on the size of the gift that is non-positive, i.e. no additional restriction beyond the restrictions needed to satisfy the low type incentive compatibility constraint.
	\item [(ii)] When $\de_h(T+D)\geq W$ and $e(T-W) <1-\de_h$, Expression~\ref{eq:med4} is a non-negative upper bound on the size of the gift.
	\item [(iii)] When $\de_h(T+D)\leq W$ and $e(T-W) >1-\de_h$, Expression~\ref{eq:med4} is a non-negative lower bound on the size of the gift, which may be larger or smaller than the bound in Expression~\ref{eq:med3}. This is an additional restriction beyond that for the low type only when the bound is larger than the one in Expression~\ref{eq:med3}.
	\item [(iv)] When $\de_h(T+D)\leq W$ and $e(T-W) <1-\de_h$, Expression~\ref{eq:med4} is a non-positive upper bound on the size of the gift and therefore there is no incentive compatible mechanism.
\end{itemize}
(c): The low type's individual rationality constraint is satisfied trivially: the low type gets the `War' payoff forever outside the mechanism; it also gets the `War' payoff forever \emph{inside} the mechanism while not giving or receiving any concession.

For the high type, we need
\begin{equation*}
...
\end{equation*}
If $Te < (1-\de_h)(1-e)$, this is a negative lower bound on the size of the concession and thus implies no additional restriction. If $Te = (1-\de_h)(1-e)$, there is again no restriction because $T+D >W$ by assumption. If $Te > (1-\de_h)(1-e)$, the high type's individual rationality constraint requires that $\frac{ T + D - W}{Te - (1-\de_h)(1-e)}>g$.

\hfill $\blacksquare$\\


As before, the optimal gift is the one where Expression~\ref{eq:med3} holds with equality [NOT NECESSARILY BECAUSE OF HIGH TYPE IC]. We call this value $g^M$. We see here that $g^M$ depends on $e$, which is also the choice of the mediator \emph{and} that we must have $\overline{e} = p - e \left( p + T - \de_l T + W \right) > 0$ in order for the optimal concession to be both positive and finite. Both the optimal concession increases toward infinity as $e$ approaches $\frac{p}{p + \left(1-\de_l\right) + W}$, which is the value that makes the denominator of $g^M$ equal to 0.

There are now two cases to consider. Depending on parameter values, welfare either increases toward infinity or decreases toward negative infinity as $e \rightarrow \overline{e}$. A sufficient condition In the welfare-maximizing mediation mechanism, the mediator sets $e$ to be as large as is feasible given the resources available to be given as concessions as long as $e < \frac{p}{p + \left(1-\de_l\right) + W}$ 
