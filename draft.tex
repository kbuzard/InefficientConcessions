...
The threshold needed is \be
 \delta^{gt} = \frac{W}{T+D}
 \ee
where the superscript $gt$ stand for `grim trigger.' Here, as in each further iteration of the model, we define a high type as a country who prefers to play the ``Peace'' equilibrium, i.e. $\delta_h^{gt} \geq \delta^{gt}$. Any country with $\delta_l^{gt} < \delta^{gt}$ is a low type. This means that two high types---and only two high types---can sustain the Peace outcome in a simple repeated game.

\section{Analysis of Benchmark Cases}
\label{sec:bench}
When countries are faced with uncertainty about each other's motives, they can proceed in several different ways, including giving concessions in order to signal their willingness and ability to make peace. There are three basic types of equilibria that this analysis will examine.\footnote{If the discount factor is high enough, there are many types of equilibria. Consider for instance oscillating every other period between Peace and War with a grim trigger War threat. In this paper, we are not interested in these equilibria because they are not as realistic for applications to conflict scenarios. These are also always payoff dominated by at least one of the other equilibria discussed and thus 	are not attractive.} The Appendix provides a thorough analysis of the various equilibria as well as all proofs for this section. 
\\
\\
...
\begin{corollary}
	The threshold patience level for a country to play Trust in a no-concessions separating equilibrium, $\de_h^{nc}$, is higher than for the grim-trigger repeated game equilibrium. 
	\label{corollary:1}
\end{corollary}
\emph{Proof}: See Appendix.
...
\be
  p\left(\frac{T}{1- \delta_h}\right) +(1-p)\left({-D}+\frac{\delta_h(W-D)}{1- \delta_h} \right)\geq p \left(T + W + \frac{\de_h}{1-\de_h}(W-D)\right) +(1-p)\frac{W-D}{1- \delta_h}
\ee
Simplifying, we have
\be
