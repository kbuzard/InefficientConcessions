...
In order to make the mechanism incentive compatible for the low types, some value of the concessions must be destroyed, i.e. $e<1$. The constraint that determines how efficient concessions can be comes from the low type incentive compatibility constraint. As in the concessions separating equilibria, this shows up in the optimal concession size.
...
\begin{theorem}
	...
	\begin{enumerate}
		...
		\item[(b)] improve welfare despite reducing the efficient of concessions when a \emph{concessions separating equilibrium with efficient gifts} is possible; or
...
\end{multline*}
Canceling the first two terms as well as the third and last terms, we have
\begin{multline*}
...
\]
The right hand side is positive. If the left hand side were negative, we would have a negative upper bound on the size of the gift. Given the requirement that the gift be non-negative, the low type's incentive constraint cannot be satisfied  in this case. Therefore we must assume that $p > e\left(p+\left(1-\de_l\right)T + W\right)$. Using this to isolate $g_h$ on the left hand side and gathering terms, we have 
\begin{equation}
...
Using the mediation mechanism $M$ with $e=0.29$ and $g=12.98$, we satisfy both the low type constraint that $g\geq \frac{\left(1 - \de_l \right)\left(T +D \right)}{p - e\left(p +(1-\de_l)T +W \right)}=9.5890411$ and the high type constraint $g\leq \frac{ T + D - W}{Te - (1-\de_h)(1-e)}=12.987013$. Welfare is 5.0768, compared to 2.82 in the \emph{concessions separating equilibrium with inefficient gifts}.
...
Using the mediation mechanism $M$ with $e=0.18$ and $g=7.7669$, we satisfy both the low type constraint the $g\geq \frac{\left(1 - \de_l \right)\left(T +D \right)}{p - e\left(p +(1-\de_l)T +W \right)}=7.143$ and the high type constraint $g\leq \frac{ T + D - W}{Te - (1-\de_h)(1-e)}=7.7669$. Welfare is 12.82, compared to 7.5 in the \emph{concessions separating equilibrium with inefficient gifts} and 7.67 in the \emph{concessions separating equilibrium with efficient gifts}.
...
Using the mediation mechanism $M$ with $e=0$ and $g=1.6$, we satisfy both the low type constraint $g\geq \frac{\left(1 - \de_l \right)\left(T +D \right)}{p - e\left(p +(1-\de_l)T +W \right)}=1.6$ and there is no high type constraint because $0=Te<(1-\de_h)(1-e)=0.0625$. Welfare is approximately 0.24, compared to 0 outside the mechanism.
	\end{enumerate}
	\hfill $\blacksquare$\\
